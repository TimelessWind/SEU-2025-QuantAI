...
CREATE TABLE StockMarketData (
    ...
    CONSTRAINT chk_high_low_logic CHECK (
        high_price >= open_price AND high_price >= close_price AND high_price >= low_price AND
...
VALUES (CONCAT('LOG_', UNIX_TIMESTAMP(), '_', FLOOR(RAND() * 1000)), 'admin_001', 'admin', 'data_sync', '数据库初始化完成', 'success');

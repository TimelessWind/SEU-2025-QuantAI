...
class StockDataManager:
    ...
    def process_stock_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 处理缺失值 - 智能填充
        # 价格类数据（使用前向填充或后向填充）
        price_columns = ['open_price', 'close_price', 'high_price', 'low_price', 'pre_close_price']
        ...
        # 涨跌额和涨跌幅（使用计算值填充）
        if 'pre_close_price' in processed_df.columns and 'close_price' in processed_df.columns:
            if 'change_amount' in processed_df.columns:
                processed_df['change_amount'] = processed_df['change_amount'].fillna(processed_df['close_price'] - processed_df['pre_close_price'])
            if 'change_percent' in processed_df.columns:
                # 避免除零错误
                mask = processed_df['pre_close_price'] != 0
                processed_df.loc[mask, 'change_percent'] = processed_df.loc[mask, 'change_percent'].fillna(
                    (processed_df.loc[mask, 'close_price'] - processed_df.loc[mask, 'pre_close_price']) / processed_df.loc[mask, 'pre_close_price']
                )
                # 处理pre_close_price为0的情况
                processed_df['change_percent'] = processed_df['change_percent'].fillna(0)
